...
def predict_monthly_future(birthdate):
    ...
    prompt = f"""
    Today is {current_date.strftime('%Y-%m-%d')}. Based on the birthdate {birthdate}, 
    predict the future for {current_month} {current_year} in Burmese language.
...
def predict_yearly_future(birthdate):
    ...
    prompt = f"""
    Today is {current_date.strftime('%Y-%m-%d')}. Based on the birthdate {birthdate}, 
    predict the future for the year {current_year} in Burmese language.
...
if __name__ == '__main__':
    app.run(host='0.0.0.0', port=6137, debug=True)
